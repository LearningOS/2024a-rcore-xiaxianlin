...
impl TaskManager {
    ...
    /// 更新任务的系统调用数量
    fn update_task_syscall_times(&self, id: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].syscall_times[id] = inner.tasks[current].syscall_times[id] + 1;
    }
}
...
/// 更新任务的系统调用数量
pub fn update_task_syscall_times(id: usize) {
    TASK_MANAGER.update_task_syscall_times(id);
}
